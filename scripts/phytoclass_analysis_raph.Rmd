...
```{r}
data %>% 
  ggplot(aes(x = chl_c3, y = fuco, colour = all_chl_a)) +
  geom_point() +
...
results <- create_complete_phytoclass_setup("min_lit_raph_diat_fuco_viol.csv",
                                            "max_lit_raph_diat_fuco_viol.csv")
...
write.csv(cluster_summary, here("summaries", "stats_min_lit_raph_diat_fuco_viol.csv"),
          row.names = FALSE)
```
...
write.csv(combined_f_matrices, here("summaries", "out_matrix_min_lit_raph_diat_fuco_viol.csv"),
          row.names = FALSE)

write.csv(pigment_variation, here("summaries", "out_variation_min_lit_raph_diat_fuco_viol.csv"),
          row.names = FALSE)
...
output_dm <- extract_class_abundances(results_list) %>%
  ...
  pivot_longer(cols = c(GA, cryp, diat, dino, hapt, dict, cyan, raph),
               names_to = "group",
               values_to = "chla") %>%
  # Join metadata
...
```{r}
h %>% 
  ggplot(aes(date, Zea/Tchla)) +
  geom_line()

h %>% 
  ggplot(aes(Viol, Chl_b)) +
  geom_point()
```
