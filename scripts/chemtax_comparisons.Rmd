...
p <- read.csv(here("outputs", "manu_min_lit_to_dino_cyano_hapt_dict.csv"))
...
join <- pc %>% 
  ...
  mutate(diff = chla_c - chla_pc,
         ...
         diff = round(diff, 2)) %>% 
  filter(date != as.Date("2021-01-27"))
...
ggsave(here("figures", "summary_lims_dict_diff.png"), plot = p,
...
# join2 %>%
#   select(-diff) %>% 
#   pivot_longer(c(chla_c, chla_pc, group_count),
#                names_to = "analysis_type",
#                values_to = "chla") %>% 
#   group_by(date, site_id, depth, group, analysis_type) %>% 
#   summarise(chla = mean(chla), .groups = "drop") %>% 
...
#            alpha = 1, color = "black", size = 1) +
#   scale_fill_npg_distinct() +
#   facet_grid(analysis_type ~., scales = "free_y") +
#   # scale_x_date(limits = as.Date(c("2024-01-01", "2024-12-31")),
#   #              expand = c(0, 0),
#   #              date_breaks = "months", date_labels = "%b") +
#   theme_bw() +
...
ggsave(here("figures", "summary_lims_dict_diff.png"),
       plot = m1,
       width = 12, height = 8, dpi = 300, bg = "white")
```
...
ggsave(here("figures", "summary_lims_dict_diff_micro.png"),
       plot = m2,
       width = 12, height = 8, dpi = 300, bg = "white")
```